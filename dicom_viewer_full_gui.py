...
class OrganDetectionClassifier:
    ...
    def classify_organ_with_ai(self, features):
        """
        تصنيف العضو بناءً على الـ features المستخرجة
        Returns: List of top 5 organs with their probabilities
        """
        ...
        try:
            ...
            top5_indices = np.argsort(all_probs)[-5:][::-1]  # Top 5 in descending order
            ...
            for rank, idx in enumerate(top5_indices, 1):
                organ_name = self.organ_classes.get(idx, 'unknown')
                confidence = all_probs[idx]
                top5_organs.append(organ_name)
                top5_confidences[organ_name] = confidence
                
                # Visual bar
                bar_length = int(confidence * 50)
                bar = '█' * bar_length + '░' * (50 - bar_length)
                print(f"  {rank}. {organ_name.capitalize():12s} | {bar} | {confidence*100:.1f}%")
